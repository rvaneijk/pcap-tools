...
my ($readlines, $readraw, $writtenlines, $filename, $numprocc);
my (@chars, @myhexval, @files);
...
opendir(DIR, "./");
@files = sort readdir(DIR); # $files[2] t/m files[@files-1]
closedir(DIR);
$argtot = @files;
$numprocc = @files-3;
...
# MAIN
#
foreach my $argnum (3 .. $argtot)
{
#    if (open(INPUTFILE,$ARGV[$argnum])) {
    if (open(INPUTFILE,$files[$argnum])) {

	$tmpval = $argnum -2;
	  print STDERR "Converting...\n";
#	  print STDERR "[$tmpval of $numfiles] $ARGV[$argnum]\n";
	  print STDERR "[$tmpval of $numprocc] $files[$argnum]\n";
	$linenumber = 0;
